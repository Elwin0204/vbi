    ...
    <div class="chart" v-for="(chart, index) in charts" :key="index">
      ...
      >
        <defs>
          ...
          <linearGradient :id="`ringlike_linear_${index}`" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" :style="{'stop-color': chart.color2,'stop-opacity': 1}" />
            <stop offset="100%" :style="{'stop-color': chart.color2,'stop-opacity': 0.1}" />
          </linearGradient>
        </defs>
        <path
          :stroke="chart.color1"
